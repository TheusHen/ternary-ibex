...
module mhx_ternary_tb_top;
  ...
  // Interface instantiation with named ports
  mhx_ternary_if vif(
    .clk(clk),
    .rst_n(rst_n)
  );
  ...
  initial begin
    string dump_waves_str;

    // Set virtual interface in config_db
    uvm_config_db#(virtual mhx_ternary_if)::set(null, "*", "vif", vif);

    // Enable waveform dumping (using $value$plusargs for proper string handling)
    if ($value$plusargs("DUMP_WAVES=%s", dump_waves_str)) begin
