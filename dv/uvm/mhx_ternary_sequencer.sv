...
class mhx_ternary_sequencer extends uvm_sequencer #(mhx_ternary_transaction);
  ...
  virtual task execute_sequence(uvm_sequence_base seq);
    ...
    `uvm_info("SEQR", $sformatf("Sequence %s executed %0d transactions", 
                                seq.get_name(), end_count - start_count), UVM_MEDIUM);
  endtask
  ...
  virtual function void report_phase(uvm_phase phase);
    ...
    `uvm_info("SEQR_STATS", $sformatf("Total transactions: %0d", transactions_sent), UVM_LOW);
    `uvm_info("SEQR_STATS", $sformatf("Ternary operations: %0d", ternary_ops_sent), UVM_LOW);
    `uvm_info("SEQR_STATS", $sformatf("Neural operations: %0d", neural_ops_sent), UVM_LOW);
    if (transactions_sent > 0) begin
      `uvm_info("SEQR_STATS", $sformatf("Ternary ratio: %0.1f%%", 
                                        (ternary_ops_sent * 100.0) / transactions_sent), UVM_LOW);
      `uvm_info("SEQR_STATS", $sformatf("Neural ratio: %0.1f%%", 
                                        (neural_ops_sent * 100.0) / transactions_sent), UVM_LOW);
    end
