...
class mhx_ternary_env extends uvm_env;
  ...
  virtual task monitor_coverage_progress();
    ...
    forever begin
      ...
      if (current_cov > last_coverage + 5.0) begin // Report significant coverage increases
        `uvm_info(
          "ENV",
          $sformatf("Coverage progress: %0.1f%% (target: %0d%%)", current_cov, cfg.coverage_goal),
          UVM_MEDIUM
        );
      end
  ...
  virtual function void check_phase(uvm_phase phase);
    ...
    if (agent.monitor.transactions_monitored < 10) begin
      `uvm_error(
        "ENV",
        $sformatf("Too few transactions monitored: %0d", agent.monitor.transactions_monitored)
      );
    end
    ...
    if (cfg.enable_scoreboard && scoreboard.mismatches > 0) begin
      `uvm_error(
        "ENV",
        $sformatf("Scoreboard detected %0d mismatches", scoreboard.mismatches)
      );
    end
  endfunction
  ...
  virtual function void report_phase(uvm_phase phase);
    ...
    `uvm_info(
      "ENV_SUMMARY",
      $sformatf("Total transactions: %0d", agent.monitor.transactions_monitored),
      UVM_LOW
    );
    ...
    if (cfg.enable_scoreboard) begin
      ...
      `uvm_info(
        "ENV_SUMMARY",
        $sformatf("Scoreboard pass rate: %0.1f%%", pass_rate),
        UVM_LOW
      );
    end
    ...
    if (cfg.enable_coverage) begin
      `uvm_info(
        "ENV_SUMMARY",
        $sformatf("Final coverage: %0.1f%%", coverage.get_coverage()),
        UVM_LOW
      );
    end
    ...
    `uvm_info(
      "ENV_SUMMARY",
      $sformatf("Errors detected: %0d", agent.monitor.errors_detected),
      UVM_LOW
    );
