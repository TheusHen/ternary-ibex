...
MhxSystem::MhxSystem(const char *ram_hier_path, int ram_size_words)
    : _ram(ram_hier_path, ram_size_words, 4) {}

int MhxSystem::Main(int argc, char **argv) {
  bool exit_app;
...
std::string MhxSystem::GetIsaString() const {
  const Vmhx_simple_system &top = _top;
  assert(top.mhx_simple_system);

  std::string base = top.mhx_simple_system->RV32E ? "rv32e" : "rv32i";

  std::string extensions;
  if (top.mhx_simple_system->RV32M)
    extensions += "m";

  extensions += "c";

  switch (top.mhx_simple_system->RV32B) {
    case 0:  // RV32BNone
      break;
...
int MhxSystem::Setup(int argc, char **argv, bool &exit_app) {
...
void MhxSystem::Run() {
  ...
  std::cout
      << "Simulation of MHX Neural T1 (Ternary-Extended Ibex) [Prototype]"
      << std::endl
      << "================================================================="
      << std::endl
      << std::endl;
...
bool MhxSystem::Finish() {
  ...
  svSetScope(svGetScopeFromName("TOP.mhx_simple_system"));

  std::cout << "\nPerformance Counters" << std::endl
            << "====================" << std::endl;
  std::cout << ibex_pcount_string(false);

  std::ofstream pcount_csv("mhx_simple_system_pcount.csv");
  pcount_csv << ibex_pcount_string(true);

  std::cout << "\nTernary Extension Status" << std::endl
            << "========================" << std::endl;
  std::cout << "MHX Neural T1 Ternary Extensions: Active (Prototype)"
            << std::endl;
