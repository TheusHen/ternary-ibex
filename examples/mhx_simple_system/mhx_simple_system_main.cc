...
public:
    MhxSimpleSystemSim(bool trace_enabled = false, 
    ...
    void set_rom_file(const std::string& file) {
        rom_file_ = file;
    }
    
    void set_ram_file(const std::string& file) {
        ram_file_ = file;
    }
    ...
    void print_status() {
        ...
        std::cout << "UART TX:  " << (dut_->uart_tx ? "1" : "0") << std::endl;
        std::cout << "Reset:    " << (dut_->IO_RST_N ? "Released" : "Active") << std::endl;
    }
    
    bool should_stop() {
        // Stop if max cycles reached
        if (tick_count_ >= max_cycles_) {
            std::cout << "Maximum simulation cycles (" << max_cycles_ << ") reached." << std::endl;
            return true;
        }
    ...
    void run() {
        ...
        std::cout << "Tracing: " << (trace_enabled_ ? "enabled" : "disabled") << std::endl;
        ...
        print_status();
        std::cout << "Simulation completed after " << tick_count_ << " cycles." << std::endl;
    }
};
...
void print_usage(const char* prog_name) {
    ...
    std::cout << "  -c, --cycles <n>     Set maximum simulation cycles (default: 1000000)" << std::endl;
...
int main(int argc, char** argv) {
    ...
    int c;
    while ((c = getopt_long(argc, argv, "htic:r:m:", long_options, nullptr)) != -1) {
        switch (c) {
            case 'h':
                print_usage(argv[0]);
                return 0;
