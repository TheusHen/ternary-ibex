...
uint32_t binary_multiply_accumulate(uint32_t *weights, uint32_t *inputs,
                                    int size) {
...
void benchmark_neural_inference() {
  ...
  for (int iter = 0; iter < BENCHMARK_ITERATIONS; iter++) {
    binary_result += binary_multiply_accumulate(binary_weights, binary_inputs,
                                                NEURAL_LAYER_SIZE);
  ...
  for (int iter = 0; iter < BENCHMARK_ITERATIONS; iter++) {
    ternary_result += ternary_multiply_accumulate(
        ternary_weights, ternary_inputs, NEURAL_LAYER_SIZE);
  ...
  if (ternary_metrics.total_cycles > 0) {
    uint32_t speedup =
        (binary_metrics.total_cycles * 100) / ternary_metrics.total_cycles;
    pcount_enable(0);
    puts("Neural Speedup: ");
    puthex(speedup);
    puts("% (");
...
void benchmark_matrix_operations() {
  ...
  for (int iter = 0; iter < BENCHMARK_ITERATIONS / 10;
       iter++) {  // Fewer iterations for matrix ops
...
void benchmark_arithmetic_throughput() {
  ...
  start_perf_measurement(&ternary_metrics);
  volatile uint32_t ternary_acc =
      encode_ternary_value((int8_t[TRITS_PER_REG]){1});
  ...
  for (int iter = 0; iter < BENCHMARK_ITERATIONS; iter++) {
    ...
    int8_t trits_acc[TRITS_PER_REG], trits_a[TRITS_PER_REG],
        trits_b[TRITS_PER_REG];
