...
static inline ternary_t __builtin_neural_multiply(ternary_t weights,
                                                  ternary_t inputs,
                                                  ternary_t bias) {
...
static inline ternary_t __builtin_neural_accumulate(ternary_t accumulator,
                                                    ternary_t weights,
                                                    ternary_t inputs) {
...
static inline ternary_t __builtin_neural_learn(ternary_t weights,
                                               ternary_t deltas) {
...
static inline int __builtin_ternary_get_trit(ternary_t value, int index) {
  ...
  switch (trit_bits) {
    ...
    case TRIT_POS:
      return 1;
    default:
      return 0; /* Invalid trit treated as zero */
  }
}

/* Set a single trit in a ternary value */
static inline ternary_t __builtin_ternary_set_trit(ternary_t value, int index,
                                                   int trit_value) {
  if (index < 0 || index >= 16)
    return value;
